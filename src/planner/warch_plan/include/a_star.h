...
class gridPathFinder
{
	private:
		...
		int uav_id;
		Vector3d _world_origin;
		GridNodePtr *** GridNodeMap;
		std::multimap<double, GridNodePtr> openSet;

	public:
		gridPathFinder( Eigen::Vector3i GL_size, Eigen::Vector3i LOC_size, int _uav_id)
		{	
		...
		double getObsDisCost(double dis, double max_value);
		void initGridNodeMap(double _resolution, Eigen::Vector3d global_xyz_l, bool _useEAstar);
