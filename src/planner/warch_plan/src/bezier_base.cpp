...
int Bernstein::setParam(int poly_order_min, int poly_order_max, double min_order)
{
	...
	{	
		...
		MatrixXd MQM;
		MatrixXd Q_g, Q_v, MGM, MVM; 	      // M' * Q * M in each block of the objective. No scale, only meta elements .
		...
		Q_g = MatrixXd::Zero(order + 1, order + 1);
		Q_v = MatrixXd::Zero(order + 1, order + 1);
		...
		if( poly_num1D > min_order_u )
		{
			for( int i = min_order_u; i < poly_num1D; i ++ )
			{
		    	for(int j = min_order_u; j < poly_num1D; j ++ )
		    	{
					...
					// mini jerk: Q_u(i,j) = (i-2)(i-1)i * (j-2)(j-1)j / (i+j-5);
		...
		// calculate Q_g, Q_v;
		{
			for( int i = 0; i < poly_num1D; i ++ )
			{
		    	for(int j = 0; j < poly_num1D; j ++ )
		    	{
		      		if(i >=1 && j >=1)
					{
						Q_v(i,j) = double(i*j) / double(i+j-1);
					}
		            Q_g(i,j) = double(1.0) / double(i+j+1);
		    	}
		  	}
		}

		// _min_order is integer, as 3.0 
		if(min_order_l == min_order_u)
			Q = Q_u;
		else
			Q = (_min_order - min_order_l) * Q_u + (min_order_u - _min_order) * Q_l;
		
		// for order, get mapping matrix M
	 	switch(order)
		{	
			case 0: 
			{
		...
		MGM = M.transpose() * Q_g * M;
		MVM = M.transpose() * Q_v * M;
		MatrixXd F  = CholeskyDecomp(Q);
		MatrixXd FM = F * M;
		MGMList.push_back(MGM);
		MQMList.push_back(MQM);
		MVMList.push_back(MVM);
		FMList.push_back(FM);
