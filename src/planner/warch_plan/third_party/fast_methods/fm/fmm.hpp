...
template < class grid_t, class heap_t = FMDaryHeap<FMCell> >  class FMM : public EikonalSolver<grid_t> {
    ...
    public:
        ...
        FMM(const char * name, HeurStrategy h = NOHEUR) 
            : EikonalSolver<grid_t>(name), heurStrategy_(h), precomputed_(false) {}
        ...
        inline void setNbrPath (std::vector<double>&& nbr_dis) 
        {
            swarm_distances_ = std::move(nbr_dis);
        }

        /** \brief Actual method that implements FMM. */
        virtual int origin_computeInternal(double max_v) 
        {
            if (!setup_)
                if(setup() == -1)
                    return -1;
            ...
            {   
                iter++;
                idxMin = narrow_band_.popMinIdx(); // narrow_band_ in order min grid_[idx].getTotalValue()

                n_neighs = grid_->getNeighbors(idxMin, neighbors_);
                grid_->getCell(idxMin).setState(FMState::FROZEN);
                for (unsigned int s = 0; s < n_neighs; ++s) // n_neighs: total max 2*dim, x +|- 1
                {
                    j = neighbors_[s]; 
        ...
        /** \brief swarm method that implements FMM. */
        virtual int computeInternal(double max_v)
        {
            ...
            { // For each initial point
                ...
                {   
                    double fv = max_v;
                    fv = grid_->getCell(i).getVelocity();
                    grid_->getCell(i).setHeuristicTime( getPrecomputedDistance(i) / fv);  
                    grid_->getCell(i).setSwarmNbrTime( getPrecomputedSwarmDistance(i) / fv);
                }
                else if (heurStrategy_ == DISTANCE){
                    grid_->getCell(i).setHeuristicTime( getPrecomputedDistance(i) );
                    grid_->getCell(i).setSwarmNbrTime( getPrecomputedSwarmDistance(i));
                }
                narrow_band_.push( &(grid_->getCell(i)) );
            }
            ...
            {   
                ...
                for (unsigned int s = 0; s < n_neighs; ++s) // n_neighs: total max 2*dim, x +|- 1
                {
                    ...
                    else 
                    {
                        ...
                        if (heurStrategy_ == TIME){
                            double fv = max_v;
                            fv = grid_->getCell(j).getVelocity();
                            grid_->getCell(j).setHeuristicTime( getPrecomputedDistance(j) / fv );
                            grid_->getCell(j).setSwarmNbrTime( getPrecomputedSwarmDistance(j) / fv);
                        }
                        else if (heurStrategy_ == DISTANCE){
                            grid_->getCell(j).setHeuristicTime( getPrecomputedDistance(j) );
                            grid_->getCell(j).setSwarmNbrTime( getPrecomputedSwarmDistance(j));
                        }
                        // Updating narrow band if necessary.
                        if (grid_->getCell(j).getState() == FMState::NARROW) 
                        {
            ...
            printf("fm iteration num = %d, vistNode = %d.", iter, iter + (int)narrow_band_.size());
            return 1;
        }

        bool isOutHorizon(unsigned int nowIdx, double horizon)
        {
            if(distances_[nowIdx] - horizon > 0.001) return true;
            else return false;
        }
        ...
        virtual double getPrecomputedSwarmDistance(const unsigned int idx) 
        {
            return swarm_distances_[idx];
        }
    ...
    private:
        ...
        std::vector<double>                             swarm_distances_;
