...
class Solver {
    ...
    public:
        ...
        virtual void setNbrPath (std::vector<double>&& nbr_dis) = 0;
        ...
        virtual void reset () { setup_ = false; grid_->clean(); }

        /** \brief Returns a pointer to the grid used. */
        grid_t* getGrid() const { return grid_; }

        virtual double getTime () const { return time_; }

        virtual void printRunInfo () const { console::warning("No run info available."); }
