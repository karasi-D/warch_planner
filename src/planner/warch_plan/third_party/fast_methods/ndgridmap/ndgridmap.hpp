...
template <class T, size_t ndims> class nDGridMap {
    ...
    public:
      ...
      /** @param dimsize constains the size of each dimension.
        ...
        inline T & operator[] (unsigned int idx) { return cells_[idx]; }
        ...
        /** \brief Returns the cell with index idx. */
        inline T & getCell (unsigned int idx) { return cells_[idx]; }
        ...
        double getMinValueInDim(unsigned int idx, unsigned int dim) 
        {
            ...
            // if (n_neighs == 1)
            //     return cells_[n_[0]].getTotalValue();
            // else
            //     return (cells_[n_[0]].getTotalValue()<cells_[n_[1]].getTotalValue()) ? cells_[n_[0]].getTotalValue() : cells_[n_[1]].getTotalValue();
        
        }
        double getMinValueInDimByStep(unsigned int idx, unsigned int dim, unsigned int step) 
        {
            n_neighs = 0; // How many neighbors obtained in that dimension.
            getNeighborsInDimByStep(idx,n_,dim,step);
        ...
        unsigned int getNeighborsByStep(unsigned int idx, std::array<unsigned int, 2*ndims> & neighs, unsigned int step) 
        {
            n_neighs = 0;
            for (unsigned int i = 0; i < ndims; ++i)
                getNeighborsInDimByStep(idx,neighs,i,step);
        ...
        void getNeighborsInDimByStep (unsigned int idx, std::array<unsigned int, 2*ndims>& neighs, unsigned int dim, unsigned int step) 
        {
            unsigned int c1,c2; // Candidate neighbors in dimension.
            if (dim == 0) {
                c1 = idx-step;
        ...
        void getNeighborsInDimByStep(unsigned int idx, std::array<unsigned int, 2>& neighs, unsigned int dim, unsigned int step) 
        {
            // Candidate neighbors in dimension.
            unsigned int c1,c2;
            if (dim == 0) {
       ...
       /** \brief Shows the coordinates from an index. */
        ...
        // inline void setNbrPathCells
        // (std::vector<std::vector<unsigned int>>&& nbr_idxs) {
        //     nbr_idxs_ = std::move(nbr_idxs);
        // }
    ...
    private:
        ...
        // /** \brief set the nbrpath cells (nbrs). */
        // std::vector<std::vector<unsigned int>> nbr_idxs_;
};
