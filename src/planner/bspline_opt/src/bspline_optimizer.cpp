...
/*    
    ...
    for (int i = 0; i < max_traj_nums; i++)
    {
      ...
      while ( cp_id < cps_.size) // seg_id < RichInfoSegs.size() ||
      {
        //cout << "A ";
...
/*
    ...
    for (size_t i = 0; i < segment_ids.size(); ++i)
    {
      ...
      if (a_star_->AstarSearch( 0.1, in, out)) // {(in-out).norm()/10+0.05}
      {
...
*/  
  ...
  double BsplineOptimizer::costFunctionRebound(void *func_data, const double *x, double *grad, const int n)
  {
    ...
    double cost;
    // ROS_WARN("begin combineCostRebound");
    opt->combineCostRebound(x, grad, cost, n);

    opt->iter_num_ += 1;
    printf("got combineCostRebound = %f, ", cost);
    return cost;
  }
...
/*
    ...
    if (flag_new_obs_valid)
    {
      ...
      for (size_t i = 0; i < segment_ids.size(); ++i)
      {
        ...
        if (a_star_->AstarSearch( 0.1, in, out)) // ({(in-out).norm()/10+0.05}0.1, in, out)
        {
          a_star_pathes.push_back(a_star_->getPath());
        }
        else
...
*/
  ...
  bool BsplineOptimizer::BsplineOptimizeTrajRebound(Eigen::MatrixXd &optimal_points, double &final_cost, const ControlPoints &control_points, double ts)
  {
    ...
    cps_ = control_points;
    // ROS_WARN("begin rebound_optimize");
    bool flag_success = rebound_optimize(final_cost);

    optimal_points = cps_.points;
    ROS_WARN("over rebound_optimize, flag_success = %d", flag_success);
  ...
  bool BsplineOptimizer::rebound_optimize(double &final_cost)
  {
    ...
    ROS_WARN("start_id = %d, end_id = %d",start_id, end_id);
    variable_num_ = 3 * (end_id - start_id);
    ROS_WARN("variable_num_ = %d", variable_num_);
    ...
    do
    {
      ...
      ROS_WARN("1. insert lbfgs_params");
      ...
      /* ---------- optimize ---------- */
      t1 = ros::Time::now();
      ROS_WARN("2. begin %dth lbfgs_optimize, final_cost = %f", rebound_times, final_cost);
          ...
          result == lbfgs::LBFGS_STOP)
      {
        ...
        flag_force_return = false;
        success = true; // add by karasi
        /*** collision check, phase 1 ***/
/* karasi make it not use!!
